...
export function ServicesPlanner() {
  ...
  const counts = useMemo(() => {
    const exp = parseInquiryCount(experian);
    const eq = parseInquiryCount(equifax);
    const tu = parseInquiryCount(transunion);
    const total = exp + eq + tu;
    const recommended = getRecommendedPlan(total);

    return { exp, eq, tu, total, recommended };
  }, [equifax, experian, transunion]);

  const highlightedPlan = counts.recommended ?? "standard";
  const recommendedPlan = counts.recommended
    ? PLANS.find((plan) => plan.key === counts.recommended) ?? null
    : null;

  return (
    <>
      <section className="services-plans" aria-labelledby="plans-heading">
            ...
            <div className="tier-calculator-grid">
              <label className="tier-field">
                ...
                <input
                  ...
                  pattern="[0-9]*"
                  value={experian}
                  onChange={(event) => setExperian(event.target.value.replace(/[^0-9]/g, ""))}
                  placeholder="0"
              ...
              <label className="tier-field">
                ...
                <input
                  ...
                  pattern="[0-9]*"
                  value={equifax}
                  onChange={(event) => setEquifax(event.target.value.replace(/[^0-9]/g, ""))}
                  placeholder="0"
              ...
              <label className="tier-field">
                ...
                <input
                  ...
                  pattern="[0-9]*"
                  value={transunion}
                  onChange={(event) => setTransunion(event.target.value.replace(/[^0-9]/g, ""))}
                  placeholder="0"
                ...
                <em>
                  {recommendedPlan ? (
                    <>
                      ...
                      <Link href={`#${recommendedPlan.id}`} className="tier-plan-link">
