...
export function ServicesPlanner() {
  ...
  const total = useMemo(() => {
    return parseInquiryCount(experian) + parseInquiryCount(equifax) + parseInquiryCount(transunion);
  }, [equifax, experian, transunion]);

  const recommendedPlan = getRecommendedPlan(total);
  const highlightedPlan: PlanKey = recommendedPlan ?? "standard";
  const recommended = recommendedPlan ? PLANS.find((plan) => plan.key === recommendedPlan) : null;
            ...
            <div className="tier-calculator-grid">
              <label className="tier-field">
                ...
                <input
                  ...
                  pattern="[0-9]*"
                  value={experian}
                  onChange={(event) => setExperian(cleanNumber(event.target.value))}
                  placeholder="0"
              ...
              <label className="tier-field">
                ...
                <input
                  ...
                  pattern="[0-9]*"
                  value={equifax}
                  onChange={(event) => setEquifax(cleanNumber(event.target.value))}
                  placeholder="0"
              ...
              <label className="tier-field">
                ...
                <input
                  ...
                  pattern="[0-9]*"
                  value={transunion}
                  onChange={(event) => setTransunion(cleanNumber(event.target.value))}
                  placeholder="0"
              ...
              <div className="tier-total" aria-live="polite">
                ...
                <strong data-tier-total>{total}</strong>
                <em data-tier-message>
                  {recommended ? (
                    <>
                      Based on your count, the{" "}
                      <a className="tier-plan-link" href={`#${recommended.id}`}>
                        {recommended.label}
                      </a>{" "}
                      is your best fit.
                    </>
