...
export function ResultsFilter() {
  const [activeKey, setActiveKey] = useState<FilterKey>("home");
  const current = FILTERS.find((filter) => filter.key === activeKey) ?? FILTERS[0];
    ...
    <>
      <div className="results-filter-controls" aria-label="Situation filters" data-results-filter>
        <div className="results-filter-desktop">
          {FILTERS.map((filter) => (
            <button
              key={filter.key}
              type="button"
              className={`results-filter-button ${activeKey === filter.key ? "results-filter-button-active" : ""}`}
        ...
        <label className="results-filter-mobile">
          ...
          <select
            value={activeKey}
            data-result-filter-select
            onChange={(event) => setActiveKey(event.target.value as FilterKey)}
          >
            {FILTERS.map((filter) => (
      ...
      <article className="results-filter-panel gradient-border" data-results-filter-panel>
        ...
        <Link href="/get-started" prefetch={true} className="results-filter-link" data-results-filter-link>
